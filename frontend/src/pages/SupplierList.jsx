...
export default function SupplierList() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        const data = await api('/supplier', { token })
        if (!cancelled) {
          setSuppliers(data || [])
        }
      } catch (err) {
        if (!cancelled) {
          setError(err instanceof Error ? err.message : 'Could not fetch suppliers')
        }
      } finally {
        if (!cancelled) {
          setLoading(false)
  ...
  async function handleDelete(id) {
    ...
    try {
      const token = await getFreshToken()
      if (!token) return
      await api(`/supplier/${id}`, { method: 'DELETE', token })
      setSuppliers((prev) => prev.filter((c) => c.id.toString() !== id.toString()))
    } catch (err) {
    ...
    <DashboardLayout activeNav="bills">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-[#111827] text-[28px] font-bold tracking-tight">Suppliers</h2>
          <button
            type="button"
                    ...
                    >
                      ...
                      <td className="py-3 align-middle">
                        <div className="flex items-center gap-3">
                          <button
                            type="button"
                            onClick={() => navigate(`/supplier/edit/${row.id}`)}
