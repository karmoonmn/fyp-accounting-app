    ...
    let cancelled = false
      ; (async () => {
        ...
        try {
          ...
          const customer = await api(`/customer/${id}`, { token })
          if (!cancelled && customer) {
            setName(customer.name || '')
            setEmail(customer.email || '')
            setPhoneNum(customer.phoneNum || '')
            setAddr(customer.addr || '')
          }
        } catch (err) {
          if (!cancelled) {
            setError(err instanceof Error ? err.message : 'Could not fetch customer details')
          }
        } finally {
          if (!cancelled) {
            setBusy(false)
        ...
        <div className="p-6">
          <form id="customer-form" onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-[#E5E7EB]">
              ...
              >
                {busy ? 'Saving...' : (isEdit ? 'Update Customer' : 'Save Customer')}
              </button>
            </div>
          </form>
        </div>
