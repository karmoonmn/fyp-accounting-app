    ...
    <div className="mx-1 my-2 overflow-hidden rounded-2xl border border-[#0F766E]/20 bg-gradient-to-b from-[#F0FDF4] to-white shadow-sm">
      ...
      <div className="px-4 py-3">
          ...
          <div className="mt-3 space-y-1.5 rounded-xl bg-[#F9FAFB] p-3 border border-[#E5E7EB]">
            {action.line_items.map((item, i) => (
              <div key={i} className="flex justify-between text-[12px] text-[#374151] items-start mb-1.5 last:mb-0">
                ...
                <span className="font-semibold shrink-0 tabular-nums mt-0.5">${(item.amount || 0).toFixed(2)}</span>
              </div>
            ))}
        ...
        <div className="grid grid-cols-2 gap-2 mt-3">
          <button
            disabled={action.status === 'completed' || action.status === 'cancelled'}
            onClick={onConfirm}
            className={`inline-flex items-center justify-center gap-1.5 rounded-xl bg-[#0F766E] px-3 py-2.5 text-[12px] font-bold text-white transition-all ${action.status === 'completed' || action.status === 'cancelled' ? 'opacity-50 cursor-not-allowed' : 'shadow-sm shadow-[#0F766E]/20 hover:bg-[#0d6d66] active:scale-[0.98]'}`}
          >
          ...
          <button
            disabled={action.status === 'completed' || action.status === 'cancelled'}
            onClick={onCancel}
            className={`inline-flex items-center justify-center gap-1.5 rounded-xl bg-white border border-[#FEE2E2] px-3 py-2.5 text-[12px] font-bold text-[#DC2626] transition-all ${action.status === 'completed' || action.status === 'cancelled' ? 'opacity-50 cursor-not-allowed' : 'shadow-sm hover:bg-[#FEF2F2] active:scale-[0.98]'}`}
          >
          ...
          <button
            ...
            className={`inline-flex items-center justify-center gap-1.5 rounded-xl bg-white border border-[#E5E7EB] px-3 py-2.5 text-[12px] font-bold text-[#374151] transition-all ${action.status === 'completed' || action.status === 'cancelled' ? 'opacity-50 cursor-not-allowed' : 'shadow-sm hover:bg-[#F9FAFB] active:scale-[0.98]'}`}
          >
...
export default function FloatingAiChat() {
  ...
  const [convId, setConvId] = useState(() => localStorage.getItem(`floating_conv_id_${companyId}`) || null)
  const [threadId, setThreadId] = useState(() => localStorage.getItem(`floating_thread_id_${companyId}`) || null)
  const [pendingAction, setPendingAction] = useState(() => {
    const saved = sessionStorage.getItem(`floating_pending_action_${companyId}`)
    if (saved) {
  ...
  useEffect(() => {
    const savedConv = localStorage.getItem(`floating_conv_id_${companyId}`)
    const savedThread = localStorage.getItem(`floating_thread_id_${companyId}`)
    setConvId(savedConv || null)
    setThreadId(savedThread || null)
    
    // DO NOT clear pendingAction here! It is already initialized from sessionStorage!
  ...
  // Fetch history on initial load if we have a convId
  useEffect(() => {
  ...
  async function send(text) {
    ...
    if (pendingAction && (pendingAction.status === 'completed' || pendingAction.status === 'cancelled')) {
      ...
      sessionStorage.removeItem(`floating_pending_action_${companyId}`)
    }
    ...
    try {
      ...
      // Save conversation state if it's new
      if (data.conversation_id && data.conversation_id !== convId) {
        setConvId(data.conversation_id)
        localStorage.setItem(`floating_conv_id_${companyId}`, data.conversation_id)
      }
      if (data.thread_id && data.thread_id !== threadId) {
        setThreadId(data.thread_id)
        localStorage.setItem(`floating_thread_id_${companyId}`, data.thread_id)
      }
      ...
      if (data.requires_confirmation && data.proposed_action) {
        ...
        sessionStorage.setItem(`floating_pending_action_${companyId}`, JSON.stringify(data.proposed_action))
      }
    } catch (err) {
  ...
  function handleNewChat() {
    ...
    setThreadId(null)
    setConvId(null)
    localStorage.removeItem(`floating_thread_id_${companyId}`)
    localStorage.removeItem(`floating_conv_id_${companyId}`)
    setPendingAction(null)
    sessionStorage.removeItem(`floating_pending_action_${companyId}`)
    setInput('')
  }
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-[80] group flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-[#0F766E] to-[#14B8A6] text-white shadow-xl shadow-[#0F766E]/30 transition-all duration-300 hover:scale-110 hover:shadow-2xl hover:shadow-[#0F766E]/40 focus:outline-none focus-visible:ring-4 focus-visible:ring-[#0F766E]/30 ${open ? 'scale-0 opacity-0 pointer-events-none' : 'scale-100 opacity-100'
          }`}
      ...
      <div
        className={`fixed inset-0 z-[90] bg-[#111827]/20 backdrop-blur-sm transition-opacity duration-300 ${open ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
          }`}
      ...
      <div
        ...
        className={`fixed top-0 right-0 bottom-0 z-[100] flex w-[420px] max-w-full flex-col overflow-hidden bg-white shadow-2xl ring-1 ring-black/[0.05] transition-transform duration-300 ease-in-out ${open ? 'translate-x-0' : 'translate-x-full'
          }`}
      >
        {/* ── Header ──────────────────────────────────────────────────── */}
