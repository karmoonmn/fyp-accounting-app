...
export default function QuickCreateCustomerModal({ isOpen, onClose, onSuccess }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const created = await api('/customer', { method: 'POST', token, body })
      onSuccess(created)
      onClose()
    } catch (err) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#111827]/40 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-[#E5E7EB] px-6 py-4">
          <h2 className="text-[18px] font-bold text-[#111827]">Quick add customer</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-6">
          ...
          <div className="mt-8 flex justify-end gap-3">
            ...
            <button type="submit" disabled={busy} className="h-10 rounded-xl bg-[#0F766E] px-5 text-[14px] font-bold text-white hover:bg-[#0F766E]/90 disabled:opacity-60">{busy ? 'Saving...' : 'Save customer'}</button>
          </div>
        </form>
      </div>
    </div>
